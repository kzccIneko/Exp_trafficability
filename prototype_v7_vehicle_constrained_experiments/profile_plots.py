...
def plot_exposure_bars(out_path: Path, rows: list[dict], title: str):
    plt = _setup()
    models = []
    for r in rows:
    ...
    if not models:
        return
    keys = [
        ("cross_exposure_ratio_gt_10p0deg", "横坡>10°"),
        ("cross_exposure_ratio_gt_15p0deg", "横坡>15°"),
        ("downhill_exposure_ratio_gt_10p0deg", "下坡>10°"),
        ("uphill_exposure_ratio_gt_10p0deg", "上坡>10°"),
    ]
...
def plot_tradeoff_scatter(out_path: Path, rows: list[dict], title: str):
    ...
    for m in sorted({r.get("模型代码", "") for r in rows}):
        ...
        y = [float(r.get("cross_exposure_ratio_gt_10p0deg", np.nan)) for r in sub]
        ax.scatter(x, y, label=m, alpha=0.75)
    ax.set_xlabel("路径长度 / m")
    ax.set_ylabel("横坡>10°暴露比例")
    ax.set_title(title)
    ax.grid(alpha=0.25)
    ax.legend()
    out_path.parent.mkdir(parents=True, exist_ok=True)
