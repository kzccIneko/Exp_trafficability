...
def cross_slope_impedance_from_tan(g_cross_tan: np.ndarray, alpha_r: float = 15.0) -> np.ndarray:
    ...
    横坡正负方向风险近似对称，因此使用 abs(arctan(g_cross_tan))。
    alpha_r 单位为“度”，表示横坡风险增长尺度。
    """
...
def unit_cost_from_impedance(R: np.ndarray, eps: float = 1e-6) -> np.ndarray:
    """
    阻碍度 R -> 单位距离代价 c_unit = -ln(P)，P=1-R。
