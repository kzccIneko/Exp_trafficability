...
Ours 纵坡+横坡方向代价：同时考虑行驶方向的纵坡阻力和横向稳定风险。
...
def write_metrics(out: Path, dem, p, q, C0, C1, Co, paths, start, goal, cell_size):
    ...
    for code in ["B0", "B1", "Ours"]:
        ...
        path_rows.append({
            "模型": MODEL_CN[code],
            "路径总代价 J": result.total_cost,
            "路径长度 L_m": result.path_length_m,
            "起终点直线距离 D_m": straight_m,
            "平均单位距离代价 J除以L": result.average_cost_per_m,
            "绕行率 L除以D": result.detour_ratio,
            "路径节点数": len(result.path),
            "公式说明": "J=Σ c_unit(i,θ_i)·l_i；L=Σl_i；平均单位距离代价=J/L；绕行率=L/D。",
        })
    with open(out / "02_路径规划指标.csv", "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=["模型", "路径总代价 J", "路径长度 L_m", "起终点直线距离 D_m", "平均单位距离代价 J除以L", "绕行率 L除以D", "路径节点数", "公式说明"])
        writer.writeheader()
    ...
    with open(out / "metrics_summary.csv", "w", newline="", encoding="utf-8-sig") as f:
        ...
        writer.writerow([])
        writer.writerow(["模型", "路径总代价J", "路径长度m", "平均单位距离代价", "绕行率", "路径节点数"])
        for r in path_rows:
            writer.writerow([r["模型"], r["路径总代价 J"], r["路径长度 L_m"], r["平均单位距离代价 J除以L"], r["绕行率 L除以D"], r["路径节点数"]])
...
**Ours 纵坡+横坡方向模型**：同时考虑车辆前进方向的纵坡阻力和车辆横向的侧倾/侧滑风险。它用于检验“沿等高线纵坡小，但横坡可能很大，因此不能简单认为低代价”的假设。
...
含义：上坡增加牵引需求；轻微下坡不增加阻碍；过陡下坡增加制动和失稳风险。
...
含义：横坡越大，车辆横向稳定风险越高。左倾和右倾风险近似对称，所以取绝对值。
...
解释：不能只看路径总代价 J。需要同时看路径长度、平均单位距离代价、绕行率和路径形态。如果 Ours 总代价高但平均单位距离代价低，说明它可能通过绕行避开高风险区域；如果 Ours 明显贴边，则要检查边界缓冲或参数 αr 是否过强。
...
def main():
    ...
    parser.add_argument("--alpha-r", type=float, default=15.0, help="横坡风险尺度，单位度。建议试 10、15、20、25。")
    ...
    if not args.no_path:
        for name, C in [("B0", C0), ("B1", C1), ("Ours", Co)]:
            ...
            paths[name] = res
            print(f"  {name}: 节点数={len(res.path)}, 总代价={res.total_cost:.3f}, 长度={res.path_length_m:.1f} m, 平均单位距离代价={res.average_cost_per_m:.6f}, 绕行率={res.detour_ratio:.3f}, 耗时={time.time()-t:.2f} s")
    else:
