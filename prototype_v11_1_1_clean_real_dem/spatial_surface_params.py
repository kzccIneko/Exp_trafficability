...
def generate_synthetic_landcover(
    ...
    seed: int = 42,
    *,
    add_demo_barrier: bool = False,
) -> np.ndarray:
    """生成可测试的合成土地覆盖。

    类别含义：10 林地、20 草地、30 裸地、40 潜在湿润/水体、50 建成区。

    重要修正（v11.1.1）：
    - 真实 DEM 实验默认不再生成演示用建成区小方块；
    - 只有显式传入 add_demo_barrier=True 时，才添加人造障碍块；
    - 这样可以避免论文图中出现与真实地物无关的白色方块。
    """
    ...
    # Demo barrier is disabled by default. Enable it only for synthetic debugging.
    if add_demo_barrier and rows > 80 and cols > 80:
...
def compute_spatial_surface_params(
    ...
    *,
    ...
    if landcover is None:
        landcover = generate_synthetic_landcover(dem, twi_n, seed=seed, add_demo_barrier=add_demo_barrier)
