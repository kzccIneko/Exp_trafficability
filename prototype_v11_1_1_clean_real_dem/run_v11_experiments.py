...
from plotting_v10 import plot_direction_cost_maps, plot_paths, plot_profiles, plot_bar_metrics, save_map_grid, plot_surface_params, nan_reduce_2d
...
```
"""
    (out / "00_README_v11_experiment.md").write_text(text, encoding="utf-8")
...
def run_v11(args):
    ...
    write_csv(out / "00_model_labels_B0_B1_V9_V10.csv", MODEL_DESCRIPTIONS)
    write_csv(out / "00_formula_passport.csv", formula_passport_rows())
    write_csv(out / "00_parameter_sources.csv", parameter_rows(vehicle, args))
    write_csv(out / "00_path_capability_metric_definitions.csv", indicator_explanations())
    write_vehicle_params_csv(out / "01_vehicle_parameters" / "01_vehicle_parameters.csv", vehicle)
    ...
    surface = compute_spatial_surface_params(
        ...
        seed=args.seed,
        add_demo_barrier=args.add_demo_barrier,
    )
    exp_surface = out / "02_spatial_surface_params_check"
    exp_surface.mkdir(parents=True, exist_ok=True)
    plot_surface_params(exp_surface / "01_wetness_and_adhesion_maps.png", surface)
    write_csv(exp_surface / "02_surface_params_metadata.csv", [{k: json.dumps(v, ensure_ascii=False) for k, v in surface["metadata"].items()}])

    fields, v10_field, v10_parts, _ = build_free_fields(dem, p, q, directions, cell_size, vehicle, surface, args)
    exp_dir = out / "03_directional_cost_and_dominant_constraint"
    exp_dir.mkdir(parents=True, exist_ok=True)
    plot_direction_cost_maps(exp_dir / "01_directional_cost_and_dominant_constraint.png", {"V10空间附着": v10_field}, v10_parts)
    min_v10 = nan_reduce_2d(v10_field, 'min')
    max_v10 = nan_reduce_2d(v10_field, 'max')
    write_csv(exp_dir / "02_directional_cost_statistics.csv", [
        {"统计量": "mean_min_cost", "数值": float(np.nanmean(min_v10))},
        {"统计量": "mean_max_cost", "数值": float(np.nanmean(max_v10))},
        {"统计量": "mean_direction_difference", "数值": float(np.nanmean(max_v10 - min_v10))},
    ])
    ...
    write_roi_csv(exp_dir / "03_roi_candidates.csv", rois)
    try:
    ...
    write_pairs_csv(exp_dir / "05_scenario_pairs.csv", pairs)

    exp = out / "04_unified_evaluation_and_hard_constraint"
    ...
    for pair in pairs:
        ...
        for model_name, cost_field, hard_lim in plan_specs:
            ...
            if result.path:
                ...
                write_csv(pair_dir / f"profile_{model_name}_V10_unified_evaluation.csv", profile)
        ...
        write_csv(pair_dir / "01_pair_unified_evaluation_metrics.csv", pair_rows)
        try:
            plot_paths(pair_dir / "02_pair_path_comparison.png", v10_field, planned_paths, pair["start"], pair["goal"], title=f"v11 pair {pair['pair_id']} 统一评价路径对比")
            plot_profiles(pair_dir / "03_pair_capability_profile.png", profiles, title=f"v11 pair {pair['pair_id']} V10统一评价剖面")
            plot_bar_metrics(pair_dir / "04_pair_high_utilization_ratios.png", pair_rows, title=f"v11 pair {pair['pair_id']} 高利用率路径比例")
        except Exception as e:
            (pair_dir / "plot_warning.txt").write_text(str(e), encoding="utf-8")

    summary = aggregate_by_model(all_rows)
    write_csv(exp / "00_v11_unified_evaluation_by_path.csv", all_rows)
    write_csv(exp / "01_v11_unified_evaluation_by_model.csv", summary)
    write_csv(exp / "02_v11_hard_constraint_by_path.csv", reach_rows)
    ...
    write_csv(exp / "03_v11_hard_constraint_by_threshold.csv", reach_summary)
    try:
        plot_bar_metrics(exp / "04_v11_all_paths_high_utilization_summary.png", all_rows, title="v11 统一评价：全部路径高利用率比例")
    except Exception as e:
...
def build_arg_parser():
    ...
    ap.add_argument("--add-demo-barrier", action="store_true", help="Only for synthetic debugging: add a small artificial built-up barrier patch. Disabled by default for real DEM experiments.")
