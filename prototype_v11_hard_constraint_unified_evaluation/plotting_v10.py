...
def plot_direction_cost_maps(path, fields, parts):
    setup_font(); C=fields['V10空间附着']; mn=np.nanmin(C,axis=2); mx=np.nanmax(np.where(np.isfinite(C),C,np.nan),axis=2); diff=mx-mn
...
def plot_paths(path, cost_map, paths, start, goal, title='路径对比'):
    ...
    base=np.nanmin(np.where(np.isfinite(cost_map),cost_map,np.nan),axis=2) if cost_map.ndim==3 else cost_map
