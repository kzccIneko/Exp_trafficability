...
输入 cost_unit_field 表示单位距离相对代价：
    经验阻碍度模型：c_unit = 1 - ln(P)
    车辆能力模型：c_unit = 1 + rho_max
路径边代价：
    C_edge = mean(c_unit along edge) * edge_length_m
注意：这里的 total_cost 是路径相对累计代价 J_plan，不是焦耳 J。

对于 16/32 邻域，某些边会跨越 2~3 个像元。为避免路径“跳过”中间高代价区域，
本模块会沿线段采样多个中间像元并计算平均单位距离相对代价。
"""
...
def astar_directional(
    ...
    edge_buffer: int = 0,
    hard_rho_limit: float | None = None,
) -> PathResult:
    ...
    while open_heap:
        ...
        for (dr, dc), edge_len_m, idx in zip(moves, move_lengths, move_dir_idx):
            ...
            # v11 硬约束模式：仅用于车辆能力利用率代价场 c_unit = 1 + rho_max。
            # 当 hard_rho_limit=1.0 时，rho_max > 1.0 的边直接视为不可通行。
            if hard_rho_limit is not None and c_unit > (1.0 + float(hard_rho_limit)):
                continue
